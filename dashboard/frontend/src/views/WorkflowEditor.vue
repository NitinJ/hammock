<template>
  <section class="space-y-5">
    ...
    <!-- Top: builder chat (full width when shown) -->
    <div v-if="showBuilder" class="surface flex flex-col min-h-[40vh]">
      <WorkflowBuilderChat
    ...
    <!-- Below: DAG (left) + Edit node (right) -->
    <div class="grid grid-cols-12 gap-5">
      <!-- Left: editor or yaml -->
      <div class="surface p-3 flex flex-col col-span-12 lg:col-span-7">
        <div
          v-if="!showYaml"
      ...
      <div class="surface p-4 flex flex-col col-span-12 lg:col-span-5">
...
onMounted(async () => {
  if (isCreate.value) {
    ...
    const copyFromSource = route.query.copy_from_source;
    if (typeof copyFrom === "string" && copyFrom.length > 0) {
      // Pick the right source endpoint:
      //   bundled / custom / undefined → /api/workflows/<name>
      //   <project-slug>                → /api/projects/<slug>/workflows/<name>
      let url = `/api/workflows/${encodeURIComponent(copyFrom)}`;
      if (
        typeof copyFromSource === "string" &&
        copyFromSource &&
        copyFromSource !== "bundled" &&
        copyFromSource !== "custom"
      ) {
        url = `/api/projects/${encodeURIComponent(copyFromSource)}/workflows/${encodeURIComponent(copyFrom)}`;
      }
      try {
