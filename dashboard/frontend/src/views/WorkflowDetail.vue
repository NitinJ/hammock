  ...
  <section class="space-y-5">
    <header class="surface p-5">
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="flex items-center gap-2">
          ...
          <RouterLink :to="copyRoute" class="btn-ghost text-xs">
            {{ copyLabel }}
          </RouterLink>
          <RouterLink
            :to="{
    ...
    <template v-else-if="workflow">
      <section class="surface p-5">
        ...
        <DagVisualizer :nodes="workflow.nodes" direction="vertical" />
      </section>
...
// "Save as new" copies the current workflow into the same source class:
//   bundled  → new custom workflow (same as before)
//   custom   → new custom workflow
//   project X → new workflow inside project X
const copyRoute = computed(() => {
  if (projectSlug.value) {
    return {
      name: "project-workflow-new",
      params: { slug: projectSlug.value },
      query: { copy_from: props.name, copy_from_source: projectSlug.value },
    };
  }
  return {
    name: "workflow-new",
    query: { copy_from: props.name, copy_from_source: effectiveSource.value },
  };
});

const copyLabel = computed(() => {
  if (projectSlug.value) return `Save as new (in ${projectSlug.value})`;
  return effectiveSource.value === "bundled" ? "Save as new (custom)" : "Save as new (custom)";
});
