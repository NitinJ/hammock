...
from hammock.engine.runner import discover_workflows
...
def _kind_for_static_node(slug: str, root: Path, node_id: str) -> str | None:
    ...
    if not snapshot.is_file():
        return None
    try:
        from hammock.engine.workflow import load_workflow
...
def _ordered_node_ids_from_workflow(slug: str, root: Path) -> list[str]:
    ...
    if not snapshot.is_file():
        return []
    try:
        from hammock.engine.workflow import load_workflow, topological_order
...
def workflow_yaml_path_for_name(name: str, root: Path | None = None) -> Path | None:
    ...
    HAMMOCK_ROOT.
    """
    from hammock.engine.runner import WORKFLOWS_DIR
