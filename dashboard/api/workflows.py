...
- **bundled** — read-only, ships in ``hammock/workflows/``.
- **custom** — user-created, cross-project, in ``<HAMMOCK_ROOT>/workflows/``.
...
``dashboard.api.project_workflows``.
"""
...
def create_workflow(body: WorkflowCreateRequest) -> dict[str, Any]:
    ...
    ``<HAMMOCK_ROOT>/workflows/<name>.yaml``. Bundled name conflict
