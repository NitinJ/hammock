...
def test_events_include_chat_derived_dispatches(client: TestClient, hammock_root: Path) -> None:
    _seed_completed_job_with_orchestrator_chat(hammock_root, "events-durable")
...
def test_events_persist_across_refetch(client: TestClient, hammock_root: Path) -> None:
    _seed_completed_job_with_orchestrator_chat(hammock_root, "events-durable-2")
