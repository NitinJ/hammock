...
def test_job_summary_orders_nodes_topologically(client: TestClient, hammock_root: Path) -> None:
    _seed_job_with_workflow_snapshot(hammock_root, "ordered-test-job")
...
def test_job_summary_falls_back_to_alphabetical_when_workflow_missing(
    ...
    job_dir = hammock_root / "jobs" / "no-workflow"
    job_dir.mkdir(parents=True)
    (job_dir / "nodes").mkdir()
    (job_dir / "job.md").write_text(
        "---\nslug: no-workflow\nworkflow: x\nstate: running\n---\n\n## Request\n\nx\n"
