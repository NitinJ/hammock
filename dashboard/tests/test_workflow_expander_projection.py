...
def test_expanded_nodes_for_returns_persisted_map(hammock_root: Path) -> None:
    _seed_expander_job(hammock_root, "exp-1")
    expanded = expanded_nodes_for("exp-1", hammock_root)
...
def test_expanded_nodes_for_empty_when_state_missing(hammock_root: Path) -> None:
    job_dir = hammock_root / "jobs" / "no-state"
    (job_dir / "nodes").mkdir(parents=True)
    (job_dir / "job.md").write_text("---\nslug: no-state\nworkflow: x\nstate: running\n---\n")
    assert expanded_nodes_for("no-state", hammock_root) == {}


def test_job_summary_interleaves_expanded_children_after_parent(
    client: TestClient, hammock_root: Path
) -> None:
    _seed_expander_job(hammock_root, "exp-2")
...
def test_job_summary_marks_parent_expander_on_children(
    ...
    _seed_expander_job(hammock_root, "exp-3")
...
def test_resolve_node_dir_finds_expanded_child(hammock_root: Path) -> None:
    _seed_expander_job(hammock_root, "exp-4")
    folder = resolve_node_dir("exp-4", "execute-plan__task-a", hammock_root)
    assert folder is not None
    assert folder.name == "task-a"
    assert folder.parent.name == "execute-plan"


def test_node_chat_endpoint_resolves_expanded_child(client: TestClient, hammock_root: Path) -> None:
    _seed_expander_job(hammock_root, "exp-5")
...
def test_node_detail_endpoint_resolves_expanded_child(
    ...
    _seed_expander_job(hammock_root, "exp-6")
...
def test_expanded_nodes_for_filesystem_first_when_state_json_missing(
    ...
    slug = "fs-first"
    job_dir = hammock_root / "jobs" / slug
    nodes_dir = job_dir / "nodes"
    nodes_dir.mkdir(parents=True)
    (job_dir / "job.md").write_text(
        "---\n"
    ...
    expanded = expanded_nodes_for(slug, hammock_root)
...
def test_expanded_nodes_for_merges_state_json_enrichment(hammock_root: Path) -> None:
