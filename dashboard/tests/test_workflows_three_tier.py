...
``<HAMMOCK_ROOT>/workflows/``) + project-specific (under a registered
...
def test_list_workflows_includes_custom_after_create(
    client: TestClient, hammock_root: Path
) -> None:
    r = client.post(
        "/api/workflows",
    ...
    assert (hammock_root / "workflows" / "team-flow.yaml").is_file()
...
def test_list_workflows_three_sources_present(
    client: TestClient, hammock_root: Path, tmp_path: Path
) -> None:
...
def test_resolve_at_submit_project_specific_wins(hammock_root: Path, tmp_path: Path) -> None:
    ...
    repo.mkdir()
    (repo / ".git").mkdir()
    proj.write_project(slug="alpha", repo_path=repo, name="alpha", root=hammock_root)

    # custom (HAMMOCK_ROOT/workflows/dup.yaml)
    custom_dir = wf_lib.custom_workflows_dir(hammock_root)
    ...
    resolved = wf_lib.resolve_at_submit("dup", root=hammock_root, project_slug="alpha")
    assert resolved is not None
    assert "from project" in resolved.read_text()


def test_resolve_at_submit_custom_when_no_project_copy(hammock_root: Path, tmp_path: Path) -> None:
    from dashboard import projects as proj
    from dashboard import workflows as wf_lib
    ...
    repo.mkdir()
    (repo / ".git").mkdir()
    proj.write_project(slug="beta", repo_path=repo, name="beta", root=hammock_root)

    custom_dir = wf_lib.custom_workflows_dir(hammock_root)
    custom_dir.mkdir(parents=True, exist_ok=True)
    (custom_dir / "team-only.yaml").write_text(_wf_yaml("team-only", "from custom"))

    resolved = wf_lib.resolve_at_submit("team-only", root=hammock_root, project_slug="beta")
    assert resolved is not None
    assert "from custom" in resolved.read_text()


def test_resolve_at_submit_falls_back_to_bundled(hammock_root: Path) -> None:
    """Bundled is the last fallback when neither project nor custom has the name."""
    from dashboard import workflows as wf_lib

    resolved = wf_lib.resolve_at_submit("fix-bug", root=hammock_root, project_slug=None)
    assert resolved is not None
    assert resolved.is_file()
    # Bundled workflows live under hammock/workflows/
    assert "hammock/workflows" in str(resolved)


def test_resolve_at_submit_returns_none_when_unknown(hammock_root: Path) -> None:
    from dashboard import workflows as wf_lib

    assert (
        wf_lib.resolve_at_submit("nonexistent-flow", root=hammock_root, project_slug=None) is None
    )
...
def test_delete_custom_works(client: TestClient, hammock_root: Path) -> None:
    client.post(
        "/api/workflows",
    ...
    assert r.status_code == 200
    assert r.json()["source"] == "custom"
    assert not (hammock_root / "workflows" / "delme-custom.yaml").is_file()
