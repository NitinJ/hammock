...
def test_human_decision_invalid_payload(client: TestClient, hammock_root: Path) -> None:
    # Set up a node dir manually
    paths.ensure_job_layout("test-slug", root=hammock_root)
    paths.ensure_node_layout("test-slug", "n", root=hammock_root)
    r = client.post(
...
def test_human_decision_writes_file(client: TestClient, hammock_root: Path) -> None:
    paths.ensure_job_layout("test-slug", root=hammock_root)
    paths.ensure_node_layout("test-slug", "n", root=hammock_root)
    r = client.post(
        "/api/jobs/test-slug/nodes/n/human_decision",
    ...
    assert r.status_code == 200
    decision_path = paths.node_human_decision("test-slug", "n", root=hammock_root)
...
def test_submit_job_then_list(client: TestClient, hammock_root: Path) -> None:
    ...
    job_md = paths.job_md(slug, root=hammock_root)
    for _ in range(40):
        if job_md.is_file():
...
def test_submit_multipart_with_artifacts(client: TestClient, hammock_root: Path) -> None:
    ...
    assert r.status_code == 200, r.text
    slug = r.json()["slug"]
    inputs = paths.inputs_dir(slug, root=hammock_root)
...
def test_submit_multipart_no_artifacts_still_works(client: TestClient, hammock_root: Path) -> None:
    r = client.post(
        "/api/jobs",
...
def test_submit_multipart_collisions_disambiguate(client: TestClient, hammock_root: Path) -> None:
