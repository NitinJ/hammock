...
def test_spawn_uses_custom_python_executable(hammock_root: Path) -> None:
    ...
    handle = mgr.spawn(job_slug="proj-feat", stage_id="s1", root=hammock_root)
...
def test_spawn_two_stages_distinct_handles(hammock_root: Path) -> None:
    mgr = MCPManager()
    h1 = mgr.spawn(job_slug="proj-feat", stage_id="s1", root=hammock_root)
    h2 = mgr.spawn(job_slug="proj-feat", stage_id="s2", root=hammock_root)
