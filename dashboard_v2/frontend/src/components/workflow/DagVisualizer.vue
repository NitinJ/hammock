<template>
  <div class="dag-visualizer w-full overflow-x-auto">
    <svg
      v-if="layout.width > 0"
      :width="layout.width"
...
const layout = computed<DagLayout>(() => {
  ...
  const width = PAD + (maxLevel + 1) * BOX_W + maxLevel * COL_GAP + PAD;
  const height = PAD + maxColRows * BOX_H + (maxColRows - 1) * ROW_GAP + PAD;
