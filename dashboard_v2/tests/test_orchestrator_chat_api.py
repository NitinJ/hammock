...
def test_orchestrator_events_returns_chronological(
    client: TestClient, hammock_v2_root: Path
) -> None:
    job_dir = _seed_job(hammock_v2_root, "j1")
...
def test_orchestrator_messages_empty_initially(client: TestClient, hammock_v2_root: Path) -> None:
    _seed_job(hammock_v2_root, "j2")
    r = client.get("/api/jobs/j2/orchestrator/messages")
    assert r.status_code == 200
    assert r.json()["messages"] == []


def test_post_orchestrator_message_appends(client: TestClient, hammock_v2_root: Path) -> None:
    _seed_job(hammock_v2_root, "j3")
    r = client.post(
    ...
    assert msg["id"] == "msg-1"
    # Disk
    queue = hammock_v2_root / "jobs" / "j3" / "orchestrator_messages.jsonl"
...
def test_post_orchestrator_message_rejects_empty(client: TestClient, hammock_v2_root: Path) -> None:
    _seed_job(hammock_v2_root, "j4")
    r = client.post(
