...
def test_submit_with_project_slug_clones_repo(
    ...
    slug = r.json()["slug"]
    job_repo = hammock_v2_root / "jobs" / slug / "repo"
    assert job_repo.is_dir(), "project repo should be cloned into job dir"
    assert (job_repo / "README.md").is_file()
    _ = repo  # silence unused


def test_submit_rejects_unknown_project_slug(client: TestClient, hammock_v2_root: Path) -> None:
    r = client.post(
        "/api/jobs",
...
"""
    ...
    assert r.status_code == 200, r.text
    slug = r.json()["slug"]
    snapshot = (hammock_v2_root / "jobs" / slug / "workflow.yaml").read_text()
