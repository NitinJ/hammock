...
**Evaluation-error policy.** Predicates parse at compile time, so syntax errors fail the plan compile. They evaluate at stage-dispatch time, where a `PredicateError` (e.g., the dotted path resolves into an artifact whose shape changed, or the artifact is missing) is a runtime bug somewhere upstream. Both `runs_if` and `loop_back.condition` default to **False** on `PredicateError` and log at `error` level. Skip-on-uncertainty is the safer side for both: a skipped `runs_if` stage surfaces its failure in the next stage's missing-input check; a non-looped `loop_back` terminates progress (visibly) rather than looping forever on broken context. The same policy applies at job-completion time — a stage whose `runs_if` raises is treated as skipped for the final-outputs check.
