...
2. **Validate the expansion** against these rules. The cheapest way is to shell out to the project's existing validator:
   ...
   ```
   Bash: .venv/bin/python -c "from hammock.engine.workflow import validate_expansion; import sys; validate_expansion(open(sys.argv[1]).read(), sys.argv[2])" $JOB_DIR/nodes/$EXPANDER_ID/expansion.yaml $EXPANDER_ID
   ```
